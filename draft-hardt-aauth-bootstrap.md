...
This document defines AAuth Bootstrap, an extension to the AAuth Protocol ([@!I-D.hardt-aauth-protocol]) that specifies how a SaaS browser agent, SaaS mobile agent, or B2B SaaS agent obtains its initial agent identity (`aauth:local@domain`), agent token, and resource token through a ceremony mediated by the user's Person Server (PS). Bootstrap establishes a binding between a user (as vouched for by the user's PS) and an agent identity minted by the vendor-operated agent server. The specification defines the `bootstrap_token`, the `/bootstrap` endpoints on the PS and agent server, the signature schemes used at each step, and renewal flows that bypass the PS after the initial binding is established.
...
- B2B extensions that carry organizational identity from an enterprise directory through the PS to the agent server.
...
- **Bootstrap**: The PS-mediated ceremony defined in this document by which a SaaS or B2B agent obtains its initial agent token and resource token. Distinct from the developer-side ceremonies used by self-hosted agents and from platform attestation enrollment.
...
- **bootstrap_token**: A short-lived JWT issued by the PS, directed at an agent server, and bound to an agent's ephemeral key via a `cnf` claim. Defined in (#bootstrap_token-issuance).
...
- The **agent server**, which verifies the `bootstrap_token`, performs attestation, records the `(user, agent)` binding, and issues the agent token and resource token.
...
Agent                              PS              Agent Server
  ...
  |----------------------------------------------------->|
  |                                 |                    |
  | { agent_token, resource_token } |                    |
  |<-----------------------------------------------------|
  |                                 |                    |
  | HTTPSig w/ agent token          |                    |
  | POST token_endpoint (PS)        |                    |
  | w/ resource_token               |                    |
  |-------------------------------->|                    |
  |                                 |                    |
  | auth_token                      |                    |
...
Agent                                          Agent Server
  ...
  |------------------------------------------------->|
  |                                                  |
  | { agent_token, resource_token } (new ephemeral)  |
  |<-------------------------------------------------|
~~~
...
- **`scope`** (REQUIRED). Space-separated list of identity scopes the agent server is requesting.
...
The agent directs the user to the interaction URL. The PS authenticates the user and presents a consent screen describing the agent server and the scopes being requested.
...
- **`scope`** (REQUIRED). Space-separated list of identity scopes granted by the user.
...
9. Issue the agent token and resource token (#token-response).
...
The agent server returns an agent token and a resource token:
...
The `resource_token` is a JWT as defined in [@!I-D.hardt-aauth-protocol] with:

- **`typ`**: `aa-resource+jwt`.
- **`iss`**: The agent server URL (acting as resource).
- **`aud`**: The PS URL.
- **`agent`**: The `aauth:local@<agent-server-domain>` identity.
- **`scope`**: The identity scopes granted by the PS.

## Completion

The agent exchanges the `resource_token` at the PS `/token` endpoint, as defined in [@!I-D.hardt-aauth-protocol], to obtain an `auth_token` carrying identity claims. The agent then calls the agent server's APIs using that `auth_token`.
...
The request body is empty. The agent server computes the enclave key thumbprint from the `jkt-jwt` header, looks up the binding, and issues fresh tokens. No `attestation` member appears in the body because the `jkt-jwt` signature itself is the proof.
...
Agent servers that support this specification MUST expose a `refresh_endpoint` as defined in (#agent-server-metadata). The endpoint issues a fresh agent token and resource token bound to a new ephemeral key.
...
4. Issue a fresh agent token and resource token bound to the new ephemeral key.
...
The tokens have the same structure as in (#token-response) but bound to the new ephemeral key.
...
- The PS has stopped vouching for the user (for example, B2B deprovisioning). In this case subsequent PS `/token` calls during bootstrap will fail, and the agent will be unable to obtain `auth_tokens`.
...
# Out-of-Band Consent Updates

After first bootstrap, the PS knows the `(user, agent_server)` consent state and holds a communication channel with the user.

This enables scope changes without agent involvement:

1. The agent server requests additional scopes via a later interaction with the PS.
2. The PS prompts the user via the established channel.
3. On approval, the PS updates its consent state for the `(user, agent_server)` pair.
4. The next `auth_token` issuance from the PS includes the new scopes.

PS implementations that wish to support out-of-band consent updates SHOULD establish a direct user communication channel at first bootstrap.
...
```json
{
  ...
  "scope": "openid profile email org groups roles",
  "tenant": "acme-corp",
...
These parameters are drawn from [@OpenID.EnterpriseExtensions] and tell the PS which organizational identity context to use.

The `bootstrap_token` and the resulting `auth_tokens` carry enterprise claims such as `org`, `groups`, and `roles` from the organization's identity infrastructure. This allows the vendor to apply organization-based authorization without running a per-customer SAML or OIDC integration.
...
- **AAuth Protocol ([@!I-D.hardt-aauth-protocol])**: Bootstrap produces an `agent_token` and `resource_token` in the shapes defined by the main specification, and consumes the HTTP Message Signatures profile defined there.
...
- **OpenID Connect Enterprise Extensions ([@OpenID.EnterpriseExtensions])**: Source of the enterprise parameters (`tenant`, `domain_hint`, `login_hint`) and claims (`org`, `groups`, `roles`) used in B2B bootstrap.
