...
- **`interaction_endpoint`** - User-facing endpoint for authentication and/or consent
...
Some AAuth capabilities represent fundamental design differences from OAuth's client-centric model: progressive authentication (pseudonym → identity → auth-token vs binary token presence), agent delegation (agent server + persistent sub across key rotations vs static client_id), and unified auth (single token with identity and authorization vs separate access/ID tokens).
...
8. **Progressive agent authentication**: Three levels (pseudonym, identity, auth-token)
...
AAuth is an exploration of agent-centric protocol design for use cases not well-served by OAuth's client-centric model. It's not a replacement for OAuth/OIDC, not backward compatible, and not an OAuth profile. AAuth learns from OAuth's proven patterns (authorization code flow, metadata) while exploring different design choices. Some concepts might inform future OAuth evolution; others represent fundamental architectural differences.
...
| **Progressive Authentication** | Not defined | Three levels: pseudonym, identity, auth-token |
...
**New endpoints:**
- `token_endpoint` - Single endpoint for all agent-to-auth-server communication (authorization requests, polling, clarification, refresh, token exchange)
- `interaction_endpoint` - User authentication and consent flow (interactive, user-facing)
...
- Bind auth tokens to agent identifier (`agent` claim) + `sub` (agent delegate identifier)
...
sequenceDiagram
    ...
    Agent->>User: redirect to user_interaction URL<br/>(with callback_url)
    User->>Resource: GET /oauth-flow?session=xyz&<br/>callback_url=...
    ...
    Resource->>User: redirect back to<br/>agent callback_url
...
5. **Recovery:** Present expired auth token with new key to obtain new auth token
...
**Common flow:** App generates installation ID and key pair → obtains platform attestation → sends to agent server → server validates attestation and issues agent token with installation-level `sub` → keys rotate without affecting auth token refresh (bound to agent + `sub`).
...
**Common flow:** User authenticates → app generates/retrieves keys → agent server issues token with installation-level `sub` → persistent identity enables auth token refresh continuity across key rotations.
...
- Long-lived sessions with auth token refresh
- Progressive authentication from pseudonym to auth-token
...
3. **Abuse mitigation** - Progressive rate limiting based on authentication level (unsigned → pseudonym → identity → auth-token)
...
1. Agent redirects user to resource's interaction endpoint (with `callback_url`)
2. Resource redirects user to authorization server
3. Authorization server redirects user back to resource
4. Resource redirects user back to agent's `callback_url`
...
Location: https://resource-r.example/auth-flow?session=xyz789&request=abc&callback_url=https://agent-a.example/callback
Redirect-Query: session=xyz789&request=abc&callback_url=https%3A%2F%2Fagent-a.example%2Fcallback
Redirect-Path: /callback
```

**Properties:**
- Redirect-Query contains the complete query string with all parameters (URL-encoded)
- Redirect-Path specifies the expected path prefix for the callback_url
...
```http
GET /auth-flow?session=xyz789&request=abc&callback_url=https://agent-a.example/callback HTTP/1.1
...
**Validation:**
1. The resource extracts the `callback_url` parameter from Redirect-Query (or falls back to query string)
2. If Redirect-Origin is present, validate that `callback_url` starts with the Redirect-Origin value
3. If Redirect-Path is present, validate that `callback_url` path starts with the Redirect-Path value
4. Combined: `callback_url` MUST start with `Redirect-Origin` + `Redirect-Path`
...
When a resource (acting as an agent) redirects a user back to the original agent's `callback_url`, the resource would also include the Redirect-Query header.
...
- Validate `callback_url` uses HTTPS
- Validate `callback_url` matches expected patterns or allowlists
...
**Key redirect (Step 2 - Resource validates agent's callback_url):**
```http
GET /auth-flow?...&callback_url=https://agent-a.example/callback HTTP/1.1
Host: resource-r.example
Redirect-Origin: https://agent-a.example
Redirect-Path: /callback
```
Resource validates: `callback_url` starts with Redirect-Origin + Redirect-Path
